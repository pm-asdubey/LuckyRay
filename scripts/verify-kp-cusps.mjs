...
// ── KP Events ──────────────────────────────────────────────────────────
if (chart.kp.events?.length) {
  console.log('\n── KP Event Promises ───────────────────────────────────────────');
  chart.kp.events.forEach(e => {
    const promised = e.isPromised ? `PROMISED (${e.promiseStrength})` : 'NOT PROMISED';
    console.log(`  ${e.topic.padEnd(12)}: ${promised} — ${e.promiseReason?.slice(0, 80) ?? ''}`);
  });
}
